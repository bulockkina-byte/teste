...
interface Props {
  apoc?: APOC | null;
  onSave: (data: Omit<APOC, 'id' | 'createdAt' | 'updatedAt' | 'funcao'>) => void;
  onClose: () => void;
}
...
export function APOCForm({ apoc, onSave, onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!nomeCompleto || !nomeGuerra || !email) {
      ...
      <div className="relative w-full max-w-xl rounded-2xl bg-white/95 shadow-2xl backdrop-blur-sm dark:bg-graphite-800/95">
        <div className="flex items-center justify-between border-b border-graphite-200/60 px-6 py-4 dark:border-graphite-700/40">
          <div>
            ...
            <p className="text-xs text-graphite-400">Centro de Operações Aeroportuárias</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5">
          {erro && (
            <div className="rounded-xl bg-red-50 px-4 py-3 text-sm text-alert-red dark:bg-red-900/20">
              {erro}
            </div>
          )}
          ...
          <div className="rounded-xl border border-aviation-200/50 bg-aviation-50/50 px-4 py-3 dark:border-aviation-700/30 dark:bg-aviation-900/20">
            <p className="text-xs font-medium text-aviation-700 dark:text-aviation-300">Função: MOTIVA (preenchido automaticamente)</p>
          </div>

          <div>
            <label className="mb-1.5 block text-sm font-medium text-graphite-700 dark:text-graphite-300">Nome Completo *</label>
            <input value={nomeCompleto} onChange={e => setNomeCompleto(e.target.value)} placeholder="Nome completo"
              className={inputClass} />
          </div>

          <div>
            <label className="mb-1.5 block text-sm font-medium text-graphite-700 dark:text-graphite-300">Nome de Guerra *</label>
            <input value={nomeGuerra} onChange={e => setNomeGuerra(e.target.value)} placeholder="Nome de guerra"
              className={inputClass} />
          </div>

          <div>
            <label className="mb-1.5 block text-sm font-medium text-graphite-700 dark:text-graphite-300">E-mail *</label>
