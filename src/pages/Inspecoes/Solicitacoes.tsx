...
import { ClipboardList, Plus, ArrowLeft, Clock, CalendarDays, Users, FileText, ChevronDown, ChevronUp, Pencil, Trash2 } from 'lucide-react';
...
export function Solicitacoes() {
  ...
  async function handleSalvar(e: React.FormEvent) {
    ...
    if (editando?.id && !canManageEquipe(editando.equipe)) {
      alert('Você só pode editar solicitações da sua equipe efetiva.');
      return;
    }
    setSaving(true);
    try {
  ...
  async function handleDelete(id: string) {
    ...
    if (!alvo || !canManageEquipe(alvo.equipe)) {
      alert('Você só pode excluir solicitações da sua equipe efetiva.');
    ...
    <PageContainer>
      <div className="mb-6 flex items-center justify-between">
        ...
        {modo === 'lista' && (canManageGlobal || equipeEfetiva) && (
          <button onClick={() => { setEditando(null); setTipoSolicitacao(''); setDescricao(''); setModo('form'); }}
      ...
      {modo === 'form' ? (
        <form onSubmit={handleSalvar} className="max-w-4xl mx-auto space-y-5">
          <div className="mb-4 flex items-center gap-3">
            ...
            <span className="text-lg font-bold text-graphite-900 dark:text-graphite-100">{editando ? 'Editar Solicitação' : 'Nova Solicitação'}</span>
          </div>
          ...
          <div className="flex items-center gap-3 justify-end">
            ...
            <button type="submit" disabled={saving || !data || !dataTurno || !hora || !equipe || !tipoSolicitacao.trim() || !descricao.trim()}
              ...
              {saving ? 'Salvando...' : (editando ? 'Salvar Alterações' : 'Registrar Solicitação')}
            </button>
          </div>
        </form>
      ) : (
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white/95 p-6 shadow-xl shadow-black/5 backdrop-blur-sm dark:bg-surface-elevated/95 dark:shadow-black/20">
            ...
            <p className="mb-6 text-sm text-graphite-500 dark:text-graphite-400">Tem certeza que deseja excluir esta solicitação? Esta ação não pode ser desfeita.</p>
